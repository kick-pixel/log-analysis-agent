...
很多人听到“日志分析智能体”，第一反应是：那就让大模型把日志读一遍、直接总结不就行了？
但现实更残酷：日志太大上下文装不下；全量喂给模型既慢又贵；更要命的是你很难复盘——线上再来一次，你可能都说不清它到底查了哪些证据、为什么会走到这个结论。

所以这期我们要做的，是一个真正**可落地**的日志分析智能体：
先把日志解析、清洗、建索引，让它变成“可检索资产”；

然后让模型像工程师一样通过工具去检索取证；

最后把证据组织成你能直接拿去汇报和落单的结构化结论：故障时间点、关键日志、根因分析、建议方案。

目标：让用户“说人话”分析系统/业务/车载/Android/Linux 日志，同时在工程侧做到**检索可控、流程可控、会话隔离、可观测、可扩展**，并兼顾**性能与隐私**。

核心代码：

- `src/agent_layer/orchestrator.py`：`LogAnalysisAgent`（Agent / Tool Loop，一键黑盒）
- `src/agent_layer/graph_orchestrator.py`：`LogAnalysisAgent`（State Graph，手写白盒）
...
| Tools                     | 时间/关键词/语义/Tag/上下文/统计            | `src/agent_layer/tools/log_tools.py`      |
...
- UI 清空 `st.session_state['messages']` 不等于清空 Graph 内部 messages
