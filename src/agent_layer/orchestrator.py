...
注意：如果无法确定根本原因，请明确说明，不要编造信息。
    ...
    def load_logs(
        ...
        try:
            ...
            logger.info(f"Indexing {len(important_entries)} important logs (W/E/F) to vector database...")
            
            if important_entries:
                self.vector_engine.insert_logs(important_entries, session_id=session_id)
            else:
                logger.warning("No ERROR/WARN logs found, skipping vector indexing")
            
            # 统计信息
            total_logs = len(processed_entries)
            vector_logs = len(important_entries)
            logger.info(f"📊 存储统计: 关键词索引={total_logs}, 向量索引={vector_logs} ({vector_logs/total_logs*100:.1f}%)")
...
def main():
    ...
    # 加载日志
    print("\n=== 加载测试日志 ===")
    load_result = agent.load_logs(str(sample_path), session_id="test_agent_session")
